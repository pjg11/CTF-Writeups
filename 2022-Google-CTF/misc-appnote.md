...
The central directory consists of:
- central directory file headers, which point to files in the archive and contain additional metadata.[^1]
- end of central directory record or EOCDR, which is used by zip tools to correctly read and extract files from the archive. This structure contains information about the archive and points to the start of the central directory, which further points to the data in the archive.[^2] [^3]
...
The document also mentioned that a ZIP file must have only one EOCDR. So, maybe checking the EOCDR segment of the ZIP file will help find out which files to extract?

## Solution

The EOCDR begins with the hex bytes `50 4B 05 06` (big-endian). After opening the file in a hex editor, I searched for the hex bytes. And...there's multiple results, specifically 21 of them?
...
The data significant for the solution is the `offset of start of central directory with respect to the starting disk number`. In short, this points to the start of the central directory. 

In the above example, the address is `0x0000880a` in big-endian, or `0x00000a88` in little-endian. I entered the address (in little-endian format) in the hex editor and it points to the central directory header (`50 4B 01 02`). 

We can see the character `C` 1 byte before the file header. As the flag begins with `CTF`, I know that this is the correct character.

![](images/appnote-char.png)

A similar process can be followed remaining records, and we get our flag! Of course, I wanted to make a script to make this process easier, so [I did](src/appnote-sol.py).
...
[^1]: https://users.cs.jmu.edu/buchhofp/forensics/formats/pkzip.html#centraldirectory
[^2]: https://www.hanshq.net/zip.html#eocdr
